function [zadoffChuSeq_slot, preambleIndex_timeAdvance_arr, array_ifft_corr_fft, ifftSeq_array, mean_u_ifft, C_v_arr, detected_preambleIndex_arr, nTA_microSecond_arr] = frequencyCombining_PRACH_demodulation_plot1(prachConfig, carrierConfig, PrachConfigFR1UnpairedSpectrum, constant, timeErrorTolerance, timeDomain_signal, isPlot, isPrintResult, u, C_v, figure_num, fileID)
% 27 December 2024
...
for multipleIndex = 1:length(multiples)
    for preamble_only_idx = 1:num_preamble_only
        ...
        prachBlockIndex_arr = reshape(1:prachDuration, [blockSize, multiple]);
        ...
        for prachSumFreqIndex = 1:multiple
            ...
            for u_index = 1:length(u_arr)
                ...
                for ncv = 0:nPreambleSeq - 1
                    ...
                    if isequal(u_arr(u_index), u) & isequal(ncv, prachConfig.PreambleIndex)
                        % fprintf('maxPeakWindow = %.2f; new_maxPeak_threshold = %.2f\n', maxPeakWindow, new_maxPeak_threshold);
                        fprintf(fileID, '\tmaxPeakWindow = %.2f; new_maxPeak_threshold = %.2f\n', maxPeakWindow, new_maxPeak_threshold);
                    end
                    if maxPeakWindow > new_maxPeak_threshold
                        ...
                        if isequal(u_arr(u_index), u) & isequal(ncv, prachConfig.PreambleIndex)
                            % fprintf('nTA_time_microSecond = %.2f; timeErrorTolerance = %.2f\n', nTA_time_microSecond, timeErrorTolerance);
                            fprintf(fileID, '\tnTA_time_microSecond = %.2f; timeErrorTolerance = %.2f\n', nTA_time_microSecond, timeErrorTolerance);
                        end

                        start_preambleIndex = length(C_v_arr) * (u_index - 1);
                        preambleIndex = start_preambleIndex + nIndex;
                        
                        %% consider time tolerance error
                        ...
                        % %% Do not consider time tolerance error
                        % detected_preambleIndex_arr = [detected_preambleIndex_arr preambleIndex];
                        % nTA_microSecond_arr = [nTA_microSecond_arr nTA_time_microSecond];
                        % %%
