...
for preIdx_index = 1:length(UE_preIdxs)
    ...
    prachConfig.PrachConfigurationIndex = randi([0 39], 1, 1);
    prachConfig.RootSequenceIndex = randi([0 837], 1, 1);

    % prachConfig.PrachConfigurationIndex = 36;
    % prachConfig.RootSequenceIndex = 821;
    ...
    % prachConfig.ZeroCorrelationZoneConfig = 0;
    ...
    %% Demodulation
    isPlot = 0;
    [zadoffChuSeq_slot, output, corr_iffft, ifftSeq_array, mean_u_ifft, C_v_arr] = PRACH_demodulation_4(prachConfig, carrierConfig, PrachConfigFR1UnpairedSpectrum, signalOut_TDL_whithoutNoise, isPlot);
    % figure(1)
