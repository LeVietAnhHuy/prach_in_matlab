function [zadoffChuSeq_slot, preambleIndex_timeAdvance_arr, array_ifft_corr_fft, ifftSeq_array, mean_u_ifft, C_v_arr] = PRACH_demodulation_4(prachConfig, carrierConfig, PrachConfigFR1UnpairedSpectrum, timeDomain_signal, isPlot, isPrintResult)
            ...
            if ismember(frame, frame_contain_prach)
                ...
                for mappingSubframe = PrachConfigFR1UnpairedSpectrum.subframeNumber
                    ...
                    for prach_index = 1:PrachConfigFR1UnpairedSpectrum.PrachDuration
                        ...
                        y_uv = preamble_only_fft_fftshift(position:(position + PrachConfigFR1UnpairedSpectrum.L_RA - 1));
                    ...
                    x_uv_avg = sum(x_uv_arr, 1) / PrachConfigFR1UnpairedSpectrum.PrachDuration;
                    ...
                    for u_index = 1:length(u_arr)
                        ...
                        if isPlot ~= 0
                            ...
                            plot(ifft_corr_fft);
                            % plot(fftshift(ifft_corr_fft));
                            % plot(flip(fftshift(ifft_corr_fft)));
                            % plot(fftshift(flip(ifft_corr_fft)));
                        ...
                        % Check for new threshold, it must be larger than
                        % initial one
                        ...
                        for ncv = 0:nPreambleSeq - 1
                            ...
                            if maxPeakWindow > new_maxPeak_threshold
                                ...
                                result = {'constant', constant, 'ifftCircShit', ifftCircShift, 'frame', frame, 'subframe', mappingSubframe, 'preIdx', preambleIndex, 'TA', nTa};
