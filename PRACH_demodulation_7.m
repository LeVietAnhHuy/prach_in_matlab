function [zadoffChuSeq_slot, preambleIndex_timeAdvance_arr, array_ifft_corr_fft, ifftSeq_array, mean_u_ifft, pos, C_v_arr] = PRACH_demodulation_7(prachConfig, carrierConfig, PrachConfigFR1UnpairedSpectrum, timeDomain_signal)
    ...
    for slot_index = 1:length(slot_contain_prach)
        ...
        for symbol_index = 1:length(start_symbol_contain_prach)
            ...
            for u_index = 1:length(u_arr)
                ...
                maxPeak_threshold = max(ifft_corr_fft) / mean(ifft_corr_fft);
                ...
                maxPeak_threshold = max(ifft_corr_fft(startI:endI)) / mean(ifft_corr_fft(startI:endI));
                [maxPeakWindow, maxPeakWindow_idx] = max(ifft_corr_fft(startI:endI));

                for ncv = 0:nPreambleSeq-2
                    x = startI:0.01:endI;
                    % Define y as a constant
                    y = maxPeak_threshold * ones(size(x));
                    % Create the plot
                    ...
                    if maxPeakWindow >= maxPeak_threshold
                        ...
                        nTa = floor((endWin - maxPeakWindow) / 8); % Timing advance
                        ...
                        start_preambleIndex = length(u_arr) * (u_index - 1);
