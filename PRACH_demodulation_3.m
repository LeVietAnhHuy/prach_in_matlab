function [zadoffChuSeq_slot, preambleIndex_timeAdvance_arr, ifft_corr_fft, pos, u_arr] = PRACH_demodulation_3(prachConfig, carrierConfig, PrachConfigFR1UnpairedSpectrum, timeDomain_signal)
    ...
    for slot_index = 1:length(slot_contain_prach)
        ...
        for symbol_index = 1:length(start_symbol_contain_prach)
            ...
            for C_v_index = 0:(length(u_arr) - 1)
                ...
                endPosWindow = startPosWindow + ceil(ifftLength / length(u_arr)) - 1;

                if C_v_index == length(u_arr) - 1
                    endPosWindow = ifftLength;
                end

                if maxPeakPosition >= startPosWindow && maxPeakPosition <= endPosWindow
                    max_C_v_index = C_v_index;
                    timingAdvance = (endPosWindow - maxPeakPosition) / 8;
                end
            ...
            %     maxPeakPosition = maxPeakPosition + windowLength;
            %     C_v_index_used = C_v_index_used + 1;
            % end
            % 
            % timingAdvance = (maxPeakPosition - 1024) / 8;
            ...
            preambleIndex = start_preambleIndex + max_C_v_index;
